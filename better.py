...
def stitch_clips(clip_paths: list, output_path: str,
                  audio_xfade_sec: float = 0.0) -> bool:
    """
    Stitch clips with STRICT 1:1 concat for BOTH video and audio.

    STRATEGY:
      1. Normalize all clips to consistent codec/fps/sample-rate/resolution.
         Clips without an audio track get a silence track via -shortest
         (no ffprobe dependency — FFmpeg terminates anullsrc when video ends).
      2. Concat ALL streams (video + audio) in a single concat-demuxer pass.
         Stream-copy first (fast, preserves quality), re-encode as fallback.
         No separate audio pipeline → zero chance of A/V desync.

    NO FFPROBE DEPENDENCY:
      Duration is parsed directly from `ffmpeg -i` stderr using regex.
      The -shortest flag is used for silence generation so no duration
      probe is needed before the silence-track normalization step.
    """
    import shutil, traceback, re
    ...
    def probe_duration(path: str) -> float:
        """
        Parse duration from `ffmpeg -i` stderr using regex.
        NO ffprobe dependency — works in imageio_ffmpeg environments
        that only ship the ffmpeg binary.
        Regex matches: Duration: HH:MM:SS.ss
        """
        r = subprocess.run(
            [ffmpeg_bin, "-i", path],
            capture_output=True, text=True,
        )
        # ffmpeg prints file info to stderr even on "error" (no output file)
        m = re.search(r"Duration:\s*(\d+):(\d+):(\d+(?:\.\d+)?)", r.stderr)
        if m:
            h, mn, s = int(m.group(1)), int(m.group(2)), float(m.group(3))
            return h * 3600 + mn * 60 + s
        # Fallback: assume standard 8-second Veo clip
    ...
    try:
        # ══════════════════════════════════════════════════════════════════════
        # Step 1: Normalize every clip to identical codec parameters
        #
        # All clips → H.264 main/yuv420p/24fps + AAC 44100Hz stereo.
        # Clips missing audio get a silence track via -shortest — FFmpeg
        # automatically terminates the infinite anullsrc when video ends.
        # No ffprobe, no duration probe needed before this step.
        # ══════════════════════════════════════════════════════════════════════
        normalized = []
        ...
        for i, p in enumerate(clip_paths):
            ...
            if has_audio:
                ...
                if r.returncode != 0:
                    raise RuntimeError(f"Normalize clip {i+1} failed:\n{r.stderr[-400:]}")
            else:
                # Use -shortest so FFmpeg terminates the infinite anullsrc
                # stream exactly when the video track ends.
                # No duration probe needed — FFmpeg handles the sync natively.
                st.write(f"  Clip {i+1}: no audio — adding silence track via -shortest")
                r = subprocess.run(
                    [ffmpeg_bin, "-y", "-i", p,
        ...
        # ══════════════════════════════════════════════════════════════════════
        # Step 2: Write concat list with absolute, forward-slash paths
        #
        # os.path.abspath ensures FFmpeg can find files regardless of the
        # working directory. replace("\\", "/") handles Windows paths in
        ...
        # ══════════════════════════════════════════════════════════════════════
        # Step 3a: Stream-copy concat (PRIMARY — fast, zero re-encode)
        #
        # All clips are already normalized to identical codec params in Step 1,
        # so stream-copy is frame-accurate and sample-accurate.
        # This is faster and perfectly preserves A/V sync vs. re-encoding.
        # ══════════════════════════════════════════════════════════════════════
        ...
        if r_copy.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 100_000:
            ...
            st.write(f"  ✅ Final video: {sz} MB, {final_dur:.2f}s (stream-copy concat, A/V locked 1:1)")
            return True

        # ══════════════════════════════════════════════════════════════════════
        # Step 3b: Re-encode concat (FALLBACK — if stream-copy fails)
        #
        # Some environments have codec profile mismatches that prevent
        # stream-copy. Re-encoding guarantees a valid output at the cost
        # of slightly longer processing time.
        # ══════════════════════════════════════════════════════════════════════
        ...
        if r_reencode.returncode == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 100_000:
            sz = os.path.getsize(output_path) // (1024 * 1024)
            final_dur = probe_duration(output_path)
            st.write(f"  ✅ Final video: {sz} MB, {final_dur:.2f}s (re-encode concat fallback)")
            return True

        st.error(f"All stitching methods failed.\nStream-copy error:\n{r_copy.stderr[-300:]}\nRe-encode error:\n{r_reencode.stderr[-300:]}")
        return False
